      ...
      return Object.keys(data).reduce((updates, key) => {
        ...
        if (current.currentVotes) {
          ...
          Object.keys(current.currentVotes).forEach(key => {
            updates[
              `${current.id}/previousVotes/${key}-${current.currentVotes[key]}`
            ] =
              current.currentVotes[key]
          })
        }
        return updates
      }, {})
    ...
    .then(updates => {
      if (Object.keys(updates).length > 0) {
        return restaurants
          ...
          .catch(() => {
            console.log(
              "An error occurred cleaning up today's votes: ",
              updates
            )
...
exports.slackVoteNotification = functions.database
  .ref('/restaurants/{restaurantId}/currentVotes/{userId}')
  .onWrite(event => {
    ...
    return Promise.all([
      ...
      const wasDeletion =
        !restaurant.currentVotes || !restaurant.currentVotes[userId]
      ...
      if (!wasDeletion) {
        postToSlack(author, restaurant)
          .then(res => {
            console.log('Slack message successfully sent: ', res)
          })
          .catch(console.error)
      }
    })
